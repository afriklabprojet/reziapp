...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <!-- En-tête -->
    <div class="mb-8">
        <a href="{{ route('profile.public', $user) }}" class="text-[#ff385c] hover:text-[#e00b41] text-sm font-medium flex items-center gap-1 mb-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                @foreach($activeBadges as $badge)
                    <div class="flex items-start gap-4 p-4 rounded-lg border-2 
                        ...
                        @elseif($badge->color === 'orange') border-[#ffb3c1] bg-[#fff0f3]
                        @elseif($badge->color === 'teal') border-teal-300 bg-[#fff0f3]
                        ...
                        <div class="shrink-0 w-12 h-12 rounded-full flex items-center justify-center
                            ...
                            @elseif($badge->color === 'orange') bg-[#ffb3c1] text-[#b5083a]
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="space-y-6">
            @foreach($allBadgeTypes as $type => $info)
                ...
                <div class="flex items-start gap-4 p-4 rounded-lg border {{ $hasBadge ? 'border-[#ffb3c1] bg-[#fff0f3]' : 'border-gray-200 bg-gray-50' }}">
                    <div class="shrink-0 w-12 h-12 rounded-full flex items-center justify-center {{ $hasBadge ? 'bg-[#ffb3c1] text-[#e00b41]' : 'bg-gray-200 text-gray-400' }}">
                        @if($info['icon'] === 'star')
                            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                            </svg>
                    ...
                    <div class="flex-1">
                        <div class="flex items-center gap-2">
                            <h3 class="font-semibold {{ $hasBadge ? 'text-[#e00b41]' : 'text-gray-900' }}">{{ $info['name'] }}</h3>
                            @if($hasBadge)
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-[#ffb3c1] text-[#b5083a]">
                                    Obtenu
                                </span>
                            @endif
                        </div>
                                ...
                                @foreach($progress as $criterion => $data)
                                    <div>
                                        ...
                                        <div class="w-full bg-gray-200 rounded-full h-1.5">
                                            <div class="bg-[#ff385c] h-1.5 rounded-full transition-all" style="width: {{ $data['percentage'] }}%"></div>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
