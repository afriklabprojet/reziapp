...
@section('title', 'Mes paiements - ReziApp')

@section('client-content')
    {{-- En-tête --}}
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-gray-900">Mes paiements</h1>
        <p class="text-gray-600">Historique de toutes vos transactions sur ReziApp</p>
    </div>
    ...
    <div class="flex flex-wrap gap-2 mb-6">
        @foreach ([
            'all'        => ['label' => 'Tous', 'color' => 'bg-[#F16A00] text-white', 'inactive' => 'bg-white text-gray-700 border border-gray-200 hover:bg-gray-50'],
            ...
            @foreach ($payments as $payment)
                @php
                    ...
                    $typeLabel = match($payment->type) {
                        ...
                        'subscription'      => 'Commission plateforme',
                        'insurance'         => 'Assurance',
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 hover:shadow-md transition">
                    <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                        {{-- Icône type --}}
                        <div class="w-12 h-12 bg-[#FFF4EB] rounded-xl flex items-center justify-center shrink-0">
                            @if ($payment->type === 'deposit')
                                <svg class="w-6 h-6 text-[#FF8A1F]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    @else
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
            ...
            <p class="text-gray-500 mb-6">
                ...
                @else
                    Vous n'avez pas encore effectué de paiement sur ReziApp.
                @endif
            </p>
            @if ($status !== 'all')
