    ...
    <div x-data="{ search: '', showNewChat: false, residenceSearch: '', selectedResidence: null, newChatMessage: '' }">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 sm:p-6 mb-5">
            <div class="flex items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    <div class="w-11 h-11 rounded-xl bg-orange-100 flex items-center justify-center">
                        <svg class="w-5.5 h-5.5 text-orange-600" fill="none" stroke="currentColor" stroke-width="1.8"
                            viewBox="0 0 24 24">
                ...
                <div class="flex items-center gap-2">
                    ...
                    @if ($residences->count())
                        <button @click="showNewChat = true"
                            class="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-semibold rounded-lg bg-orange-500 text-white hover:bg-orange-600 transition-colors shadow-sm">
                    ...
                    <a href="{{ route('chat.index', ['archived' => !$archived]) }}"
                        class="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-medium rounded-lg transition-colors
                          {{ $archived ? 'bg-orange-500 text-white hover:bg-orange-600' : 'text-gray-600 bg-gray-100 hover:bg-gray-200' }}">
                        @if ($archived)
            ...
            <div class="relative mt-4">
                ...
                <input type="text" x-model="search" placeholder="Rechercher par nom…"
                    class="w-full pl-10 pr-4 py-2.5 text-sm bg-gray-50 border border-gray-200 rounded-lg placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-400 transition-all">
            </div>
        </div>
        ...
        @if ($conversations->isEmpty())
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-10 sm:p-16 text-center">
                ...
                @if (!$archived)
                    <a href="{{ route('home') }}"
                        class="inline-flex items-center gap-2 px-5 py-2.5 bg-orange-500 text-white text-sm font-semibold rounded-lg hover:bg-orange-600 transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                @foreach ($conversations as $conversation)
                    ...
                    <a href="{{ route('chat.show', $conversation) }}"
                        ...
                        class="flex items-center gap-3.5 px-4 py-3.5 sm:px-5 border-b border-gray-50 last:border-b-0 hover:bg-orange-50/40 active:bg-orange-50/70 transition-colors relative group">
                        ...
                        <div class="relative shrink-0">
                            ...
                            @else
                                <div
                                    class="w-12 h-12 rounded-full bg-linear-to-br from-orange-400 to-orange-600 flex items-center justify-center text-white font-bold text-sm shadow-sm">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center justify-between mb-0.5">
                                <div class="flex items-center gap-1.5 min-w-0">
                                    ...
                                    @if ($other->isOwner())
                                        <span
                                            class="shrink-0 px-1.5 py-px rounded text-[9px] font-bold uppercase tracking-wider bg-orange-100 text-orange-600">Hôte</span>
                                    @endif
                                </div>
                                <span
                                    class="text-[11px] ml-3 shrink-0 {{ $unread > 0 ? 'text-orange-600 font-semibold' : 'text-gray-400' }}">
                            ...
                            <div class="flex items-center justify-between gap-2">
                                ...
                                <div class="flex items-center gap-1.5 shrink-0">
                                    ...
                                    @if ($unread > 0)
                                        <span
                                            class="inline-flex items-center justify-center min-w-5 h-5 px-1.5 text-[10px] font-bold text-white bg-orange-500 rounded-full">
                                            {{ $unread > 99 ? '99+' : $unread }}
                                        </span>
                                    @endif
                                </div>
                    ...
                    <form method="POST" action="{{ route('chat.start') }}">
                        ...
                        <div class="px-5 py-4 space-y-4">
                            ...
                            <div>
                                ...
                                <div class="relative mb-2">
                                    ...
                                    <input type="text" x-model="residenceSearch"
                                        ...
                                        class="w-full pl-10 pr-4 py-2.5 text-sm bg-gray-50 border border-gray-200 rounded-xl placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-400 transition-all">
                                </div>
                                <div
                                    ...
                                    @foreach ($residences as $residence)
                                        <label
                                            ...
                                            :class="selectedResidence == {{ $residence->id }} ? 'bg-orange-50' :
                                                'hover:bg-gray-50'">
                            ...
                            <div x-show="selectedResidence" x-transition>
                                ...
                                <textarea name="message" x-model="newChatMessage" rows="3"
                                    ...
                                    class="w-full px-4 py-2.5 text-sm bg-gray-50 border border-gray-200 rounded-xl resize-none placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-400 focus:bg-white transition-all"></textarea>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-end gap-2 px-5 py-4 border-t border-gray-100 bg-gray-50/50">
                            ...
                            <button type="submit" :disabled="!selectedResidence"
                                ...
                                :class="selectedResidence ? 'bg-orange-500 text-white hover:bg-orange-600 shadow-sm' :
