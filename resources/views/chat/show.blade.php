        ...
        <div class="hidden lg:flex flex-col w-80 border-r border-gray-100 bg-gray-50/50">
            ...
            <div class="flex items-center gap-2 px-4 py-3.5 border-b border-gray-100">
                <a href="{{ route('chat.index') }}"
                    class="flex items-center gap-1.5 text-sm font-semibold text-gray-700 hover:text-orange-600 transition-colors">
                ...
                @foreach ($conversations as $conv)
                    ...
                    <a href="{{ route('chat.show', $conv) }}"
                        class="flex items-center gap-3 px-4 py-3 border-b border-gray-50 transition-all
                          {{ $isActive ? 'bg-orange-50 border-l-2 border-l-orange-500' : 'hover:bg-gray-100/60' }}">
                        <div class="relative shrink-0">
                            @if ($convOther->avatar)
                                <img src="{{ $convOther->getAvatarUrl() }}" class="w-10 h-10 rounded-full object-cover" alt="Avatar">
                            @else
                                <div
                                    class="w-10 h-10 rounded-full bg-linear-to-br from-orange-400 to-orange-600 flex items-center justify-center text-white font-bold text-xs">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center justify-between">
                                <span
                                    class="text-sm font-semibold truncate {{ $isActive ? 'text-orange-700' : ($convUnread > 0 ? 'text-gray-950' : 'text-gray-800') }}">
                                    {{ $convOther->name }}
                                </span>
                                <span
                                    class="text-[10px] shrink-0 ml-2 {{ $convUnread > 0 ? 'text-orange-600 font-semibold' : 'text-gray-400' }}">
                            ...
                            <div class="flex items-center justify-between gap-2">
                                ...
                                @if ($convUnread > 0)
                                    <span
                                        class="shrink-0 inline-flex items-center justify-center min-w-4 h-4 px-1 text-[9px] font-bold text-white bg-orange-500 rounded-full">
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <div class="flex items-center gap-3 px-4 sm:px-5 py-3 border-b border-gray-100 bg-white">
                ...
                <div class="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    @else
                        <div
                            class="w-10 h-10 rounded-full bg-linear-to-br from-orange-400 to-orange-600 flex items-center justify-center text-white font-bold text-sm ring-2 ring-orange-100">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        @foreach ($conversation->sharedDocuments as $doc)
                            <a href="{{ Storage::url($doc->file_path) }}" target="_blank"
                                class="group flex items-center gap-2.5 p-2.5 rounded-xl hover:bg-orange-50/60 bg-white border border-gray-100 hover:border-orange-200 shadow-sm transition-all">
                                <div
                                    class="w-9 h-9 rounded-lg bg-orange-100 group-hover:bg-orange-200 flex items-center justify-center shrink-0 transition-colors">
                                ...
                                <div class="min-w-0 flex-1">
                                    <p
                                        class="text-sm font-medium text-gray-700 group-hover:text-orange-700 truncate transition-colors">
            ...
            <div x-show="showTemplates" x-transition
                class="border-t border-gray-100 bg-linear-to-b from-orange-50/40 to-white px-4 sm:px-5 py-3 max-h-40 overflow-y-auto">
                <div class="flex items-center justify-between mb-2.5">
                    <div class="flex items-center gap-2">
                ...
                <div class="flex flex-wrap gap-1.5">
                    @foreach ($templates as $template)
                        <button @click="useTemplate({{ $template->id }}, @js($template->content))"
                            class="group inline-flex items-center gap-2 px-3.5 py-2 text-sm font-medium text-gray-600 bg-white hover:bg-orange-50 hover:text-orange-700 rounded-xl border border-gray-200/80 hover:border-orange-300 shadow-sm hover:shadow transition-all active:scale-95">
                            <svg class="w-3.5 h-3.5 text-gray-300 group-hover:text-orange-500 transition-colors shrink-0"
                                fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round"
    ...
    $qr->formatMessage([
        ...
        'checkout_time' => '11h00',
            ...
            <div x-show="replyTo" x-transition.scale.y.origin.bottom
                class="border-t border-orange-200/60 bg-linear-to-r from-orange-50 to-amber-50/50 px-4 sm:px-5 py-2.5">
                <div class="flex items-center gap-3">
                    <div class="w-1 h-8 bg-linear-to-b from-orange-400 to-orange-500 rounded-full shrink-0"></div>
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-1.5">
                            ...
                            <p class="text-xs font-bold text-orange-600 uppercase tracking-wider">Réponse à</p>
                        </div>
                    ...
                    <div class="flex items-center gap-1 px-3 py-2 border-b border-gray-100 bg-gray-50/50">
                        <template x-for="(cat, idx) in emojiCategories" :key="idx">
                            <button @click="emojiTab = idx" class="p-1.5 rounded-lg text-lg transition-all"
                                :class="emojiTab === idx ? 'bg-orange-100 scale-110' : 'hover:bg-gray-100'"
                        ...
                        <div class="grid grid-cols-8 sm:grid-cols-10 gap-0.5">
                            <template x-for="emoji in emojiCategories[emojiTab]?.emojis || []" :key="emoji">
                                <button @click="insertEmoji(emoji)"
                                    class="w-9 h-9 flex items-center justify-center text-xl rounded-lg hover:bg-orange-50 hover:scale-125 active:scale-95 transition-all cursor-pointer"
                                    x-text="emoji">
                                </button>
                            </template>
                        </div>
                ...
                <div class="flex items-end gap-2">
                    ...
                    <div class="flex items-center shrink-0 pb-0.5">
                        ...
                        <button
                            ...
                            :class="showEmojiPicker ? 'text-orange-600 bg-orange-50' :
                        ...
                        {{-- Attach --}}
                        <label
                            class="group relative p-2 text-gray-400 hover:text-orange-500 rounded-xl cursor-pointer transition-all hover:bg-orange-50 active:scale-90"
                        ...
                        @if ($templates->count())
                            <button @click="showTemplates = !showTemplates; showQuickReplies = false"
                                class="p-2 rounded-xl transition-all active:scale-90"
                                :class="showTemplates ? 'text-orange-600 bg-orange-50' :
                                    'text-gray-400 hover:text-orange-500 hover:bg-orange-50'"
                    ...
                    {{-- Message input card --}}
                    <div
                        class="flex-1 flex items-end bg-white border border-gray-200 rounded-2xl shadow-sm focus-within:border-orange-400 focus-within:ring-2 focus-within:ring-orange-500/10 focus-within:shadow-md transition-all overflow-hidden">
                    ...
                    <div class="shrink-0 pb-0.5">
                        <button @click="sendMessage()" :disabled="!newMessage.trim() || sending"
                            ...
                            :class="newMessage.trim() && !sending ?
                                'bg-linear-to-br from-orange-500 to-orange-600 text-white shadow-md shadow-orange-500/25 hover:shadow-lg hover:shadow-orange-500/30 hover:from-orange-600 hover:to-orange-700 active:scale-90' :
