...
<div x-data="filterModal()" x-cloak>
    ...
    <button @click="open = true"
        class="fixed bottom-20 right-4 z-30 md:hidden w-14 h-14 bg-[#CC5A00] text-white rounded-full shadow-lg flex items-center justify-center active:scale-95 transition-transform">
        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" />
        </svg>
        <span x-show="activeFiltersCount > 0"
            class="absolute -top-1 -right-1 w-5 h-5 bg-white text-[#CC5A00] text-xs font-bold rounded-full flex items-center justify-center"
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
        ...
        <div class="sticky top-0 bg-white border-b z-10">
            <div class="flex items-center justify-between px-4 h-14">
                ...
                <button @click="resetFilters()" class="text-[#CC5A00] font-medium text-sm">
                    Réinitialiser
                </button>
            </div>
        </div>

        {{-- Contenu scrollable — laisse place au header (56px) + footer (72px) + safe area --}}
        <div class="overflow-y-auto overscroll-contain" style="height: calc(100dvh - 56px - 72px - env(safe-area-inset-bottom, 0px));">
            <div class="p-4 space-y-6">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="flex-1">
                            ...
                            <div class="relative mt-1">
                                <input type="number" inputmode="numeric" x-model="filters.price_min" placeholder="0"
                                    class="w-full pl-3 pr-12 py-2.5 border border-gray-200 rounded-xl focus:border-[#F16A00] focus:ring-[#F16A00]">
                                <span
                        ...
                        <div class="flex-1">
                            ...
                            <div class="relative mt-1">
                                <input type="number" inputmode="numeric" x-model="filters.price_max" placeholder="∞"
                                    class="w-full pl-3 pr-12 py-2.5 border border-gray-200 rounded-xl focus:border-[#F16A00] focus:ring-[#F16A00]">
                                <span
                                    class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">FCFA</span>
                            </div>
                ...
                <div>
                    ...
                    <select x-model="filters.commune"
                        class="w-full py-3 px-4 border border-gray-200 rounded-xl focus:border-[#F16A00] focus:ring-[#F16A00]">
                        ...
                        @foreach ($amenities as $amenity)
                            <label class="flex items-center gap-3 p-3 bg-gray-50 rounded-xl cursor-pointer">
                                <input type="checkbox" value="{{ $amenity->id }}" x-model="filters.amenities"
                                    class="w-5 h-5 rounded text-[#CC5A00] focus:ring-[#F16A00] border-gray-300">
        ...
        <div class="fixed bottom-0 inset-x-0 bg-white border-t p-4" style="padding-bottom: max(1rem, env(safe-area-inset-bottom));">
            <button @click="applyFilters()"
                class="w-full py-3.5 bg-[#CC5A00] text-white font-semibold rounded-xl active:bg-[#A34700] transition-colors">
