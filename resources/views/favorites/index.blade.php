        ...
        @if ($collections->isNotEmpty())
            <div class="flex flex-wrap gap-2 mb-6">
                <a href="{{ route('favorites.index') }}"
                    class="px-4 py-2 rounded-full text-sm font-medium transition-colors {{ !$collectionId ? 'bg-orange-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                    Tous
                </a>
                @foreach ($collections as $collection)
                    <a href="{{ route('favorites.index', ['collection' => $collection->id]) }}"
                        class="px-4 py-2 rounded-full text-sm font-medium transition-colors {{ $collectionId == $collection->id ? 'bg-orange-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                            ...
                            <div class="mt-3 pt-3 border-t border-gray-100">
                                ...
                                <div x-show="showNotes" x-transition class="mt-2">
                                    <form action="{{ route('favorites.note', $favorite) }}" method="POST">
                                        ...
                                        <button type="submit"
                                            class="mt-2 text-sm bg-orange-600 text-white px-3 py-1 rounded-lg hover:bg-orange-700 transition-colors">
                                            Enregistrer
                                        </button>
                                    </form>
                                </div>
