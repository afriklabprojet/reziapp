...
@section('title', 'Mes notifications - Rezi Studio Meublé Faya')
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            @if ($unreadCount > 0)
                <form action="{{ route('notifications.read-all') }}" method="POST">
                    @csrf
                    <button type="submit" class="text-sm text-[#CC5A00] hover:text-[#A34700] font-medium">
        ...
        @else
            ...
            <div class="bg-white rounded-2xl shadow-sm overflow-hidden divide-y divide-gray-100">
                @foreach ($notifications as $notification)
                    <div class="relative {{ is_null($notification->read_at) ? 'bg-[#FFF4EB]' : '' }}">
                        <a href="{{ $notification->action_url ?? '#' }}"
                            ...
                            @if (is_null($notification->read_at))
                                <div class="shrink-0">
                                    <span class="w-2 h-2 bg-[#CC5A00] rounded-full block"></span>
                                </div>
                            @endif
                        </a>
                        ...
                        <div class="absolute top-4 right-4 flex items-center gap-1">
                            @if (is_null($notification->read_at))
                                <form action="{{ route('notifications.read', $notification) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="p-1 text-gray-400 hover:text-[#CC5A00] transition-colors"
