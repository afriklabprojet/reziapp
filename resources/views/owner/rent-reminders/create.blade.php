...
@section('title', 'Nouvelle relance — Rezi Studio Meublé Faya')
...
<div class="max-w-2xl">
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-900 mt-2">Nouvelle relance de paiement</h1>
    </div>
    ...
    <form method="POST" action="{{ route('owner.rent-reminders.store') }}" class="bg-white rounded-2xl border border-gray-100 p-6 space-y-5">
        ...
        <div>
            ...
            <select name="residence_id" required class="w-full rounded-xl border-gray-200 text-sm py-2.5 focus:ring-[#F16A00] focus:border-[#F16A00]">
        ...
        <div>
            ...
            <input type="number" name="tenant_id" value="{{ old('tenant_id') }}" required class="w-full rounded-xl border-gray-200 text-sm py-2.5 focus:ring-[#F16A00] focus:border-[#F16A00]" placeholder="ID du locataire">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" name="amount" value="{{ old('amount') }}" required min="0" class="w-full rounded-xl border-gray-200 text-sm py-2.5 focus:ring-[#F16A00] focus:border-[#F16A00]">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Date d'échéance *</label>
                <input type="date" name="due_date" value="{{ old('due_date') }}" required class="w-full rounded-xl border-gray-200 text-sm py-2.5 focus:ring-[#F16A00] focus:border-[#F16A00]">
            </div>
        </div>
        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">Canal de notification *</label>
            <select name="channel" required class="w-full rounded-xl border-gray-200 text-sm py-2.5 focus:ring-[#F16A00] focus:border-[#F16A00]">
        ...
        <div>
            ...
            <textarea name="notes" rows="2" class="w-full rounded-xl border-gray-200 text-sm py-2.5 focus:ring-[#F16A00] focus:border-[#F16A00]">{{ old('notes') }}</textarea>
        </div>
