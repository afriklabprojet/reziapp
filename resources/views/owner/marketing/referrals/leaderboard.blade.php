...
@section('owner-content')
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        @if ($userRank)
            <div
                class="bg-linear-to-br from-orange-500 via-orange-600 to-orange-700 rounded-2xl p-6 text-white relative overflow-hidden shadow-lg">
                <div class="absolute top-0 right-0 w-32 h-32 bg-white/5 rounded-full -translate-y-1/2 translate-x-1/2">
                </div>
                <div class="relative flex items-center justify-between">
                    <div>
                        <p class="text-orange-100 text-sm mb-1">Votre position</p>
                    ...
                    <div class="text-right">
                        <p class="text-orange-100 text-sm mb-1">Parrainages récompensés</p>
        ...
        @else
            <div class="bg-gray-50 rounded-2xl p-8 border border-gray-200 text-center">
                ...
                <a href="{{ route('owner.marketing.referrals.index') }}"
                    class="inline-flex items-center gap-2 px-4 py-2.5 bg-orange-600 text-white text-sm font-medium rounded-xl hover:bg-orange-700 transition shadow-sm">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @else
                <ul class="divide-y divide-gray-100">
                    @foreach ($leaderboard as $index => $user)
                        <li
                            class="px-6 py-4 {{ $user->id === auth()->id() ? 'bg-orange-50/50' : '' }} hover:bg-gray-50/50 transition-colors">
                            <div class="flex items-center">
                                ...
                                <div class="flex-1 flex items-center gap-3 ml-3">
                                    <div
                                        class="w-10 h-10 {{ $user->id === auth()->id() ? 'bg-orange-100' : 'bg-gray-100' }} rounded-full flex items-center justify-center">
                                        <span
                                            class="{{ $user->id === auth()->id() ? 'text-orange-600' : 'text-gray-600' }} font-semibold text-sm">{{ substr($user->name, 0, 1) }}</span>
                                    </div>
                                    <div>
                                        <p class="font-medium text-gray-900 text-sm">
                                            ...
                                            @if ($user->id === auth()->id())
                                                <span
                                                    class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-orange-100 text-orange-700 ml-1">Vous</span>
                                            @endif
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div class="text-right shrink-0">
                                    <p class="text-lg font-bold {{ $index < 3 ? 'text-orange-600' : 'text-gray-700' }}">
