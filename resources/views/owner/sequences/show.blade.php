...
@section('title', $sequence->name . ' — REZI')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex gap-2">
            <form action="{{ route('owner.sequences.toggle', $sequence) }}" method="POST">
                @csrf
                <button type="submit" class="inline-flex items-center gap-2 px-4 py-2.5 {{ $sequence->is_active ? 'bg-orange-50 text-orange-700 hover:bg-orange-100' : 'bg-green-50 text-green-700 hover:bg-green-100' }} font-semibold rounded-xl transition-all text-sm">
        ...
        <div class="space-y-4">
            ...
            <div class="border border-gray-100 rounded-xl p-4">
                <div class="flex items-start justify-between gap-4">
                    ...
                    <form action="{{ route('owner.sequences.remove-step', [$sequence, $step]) }}" method="POST" onsubmit="return confirm('Supprimer cette étape ?')">
                        @csrf @method('DELETE')
