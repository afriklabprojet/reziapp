    ...
    <form method="POST" action="{{ route('owner.expenses.store') }}" enctype="multipart/form-data" class="bg-white rounded-2xl border border-gray-100 p-6 space-y-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="date" name="expense_date" id="expense_date" value="{{ old('expense_date', now()->format('Y-m-d')) }}" required class="w-full rounded-xl border-gray-200 text-sm py-2.5 focus:ring-orange-500 focus:border-orange-500">
        ...
        <div>
            ...
            <input type="text" name="description" id="description" value="{{ old('description') }}" required placeholder="Ex: Facture SODECI mars 2025" class="w-full rounded-xl border-gray-200 text-sm py-2.5 focus:ring-orange-500 focus:border-orange-500">
            @error('description') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
        </div>

        <div>
            <label for="amount" class="block text-sm font-semibold text-gray-700 mb-1">Montant (FCFA) *</label>
            <input type="number" name="amount" id="amount" value="{{ old('amount') }}" required min="0" step="1" placeholder="0" class="w-full rounded-xl border-gray-200 text-sm py-2.5 focus:ring-orange-500 focus:border-orange-500">
            @error('amount') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
        </div>

        <div x-data="{ recurring: {{ old('is_recurring') ? 'true' : 'false' }} }">
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_recurring" value="1" x-model="recurring" class="rounded border-gray-300 text-orange-500 focus:ring-orange-500">
            ...
            <div x-show="recurring" x-transition class="mt-3">
                ...
                <select name="frequency" id="frequency" class="w-full rounded-xl border-gray-200 text-sm py-2.5 focus:ring-orange-500 focus:border-orange-500">
        ...
        <div>
            ...
            <textarea name="notes" id="notes" rows="2" placeholder="Notes complémentaires..." class="w-full rounded-xl border-gray-200 text-sm py-2.5 focus:ring-orange-500 focus:border-orange-500">{{ old('notes') }}</textarea>
        </div>
