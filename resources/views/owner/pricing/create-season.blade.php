        ...
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                <nav class="flex items-center gap-2 text-sm text-gray-500 mb-4">
                    <a href="{{ route('owner.pricing.index', $residence) }}" class="hover:text-orange-500">← Retour au
                        calendrier</a>
                </nav>
            ...
            <form action="{{ route('owner.pricing.store-season', $residence) }}" method="POST"
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name') }}"
                            ...
                            class="w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 {{ $errors->has('name') ? 'border-red-500' : 'border-gray-300' }}">
                        @error('name')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" name="start_date" id="start_date" value="{{ old('start_date') }}"
                                ...
                                class="w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 {{ $errors->has('start_date') ? 'border-red-500' : 'border-gray-300' }}">
                            @error('start_date')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div>
                            ...
                            <input type="date" name="end_date" id="end_date" value="{{ old('end_date') }}"
                                class="w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 {{ $errors->has('end_date') ? 'border-red-500' : 'border-gray-300' }}">
                            @error('end_date')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="bg-orange-50 rounded-xl p-4">
                        <h3 class="font-medium text-orange-700 mb-4">Tarification</h3>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input type="number" name="price_per_night" id="price_per_night"
                                    ...
                                    step="100"
                                    class="w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 {{ $errors->has('price_per_night') ? 'border-red-500' : 'border-gray-300' }}">
                        ...
                        <div x-show="showAdvanced" x-collapse class="mt-4 space-y-4">
                            ...
                            <div>
                                ...
                                <textarea name="notes" id="notes" rows="2" placeholder="Notes visibles uniquement par vous..."
                                    class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500">{{ old('notes') }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex gap-4 mt-8 pt-6 border-t border-gray-200">
                    ...
                    <button type="submit"
                        class="flex-1 px-6 py-3 bg-orange-500 text-white rounded-xl hover:bg-orange-600 transition">
                        Créer la saison
                    </button>
                </div>
            </form>
