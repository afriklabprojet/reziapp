...
@section('title', 'Modifier la réponse automatique - Rezi Studio Meublé Faya')
        ...
        <form action="{{ route('owner.auto-replies.update', $autoReply) }}" method="POST" class="space-y-6"
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                <textarea name="message" rows="6" required placeholder="Écrivez votre message automatique ici..."
                    class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-[#F16A00] focus:border-[#F16A00] transition resize-y">{{ old('message', $autoReply->message) }}</textarea>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div class="text-center p-3 bg-gray-50 rounded-xl">
                        ...
                        <p class="text-sm font-bold {{ $autoReply->is_active ? 'text-[#CC5A00]' : 'text-gray-400' }}">
