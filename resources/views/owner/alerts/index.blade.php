...
@section('title', 'Alertes — Rezi Studio Meublé Faya')
...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button @click="$dispatch('open-settings')" class="inline-flex items-center gap-2 px-4 py-2.5 bg-gray-100 text-gray-700 font-semibold rounded-xl hover:bg-gray-200 text-sm">
...
{{-- Settings Modal --}}
<div id="settings-modal" x-data="{ open: false }" x-show="open" x-cloak class="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4"
     @open-settings.window="open = true" @click.self="open = false" @keydown.escape.window="open = false">
    <div class="bg-white rounded-2xl max-w-md w-full p-6">
        ...
        <form action="{{ route('owner.alerts.update-settings') }}" method="POST" class="space-y-4">
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-gray-100">
                ...
                <button type="button" @click="open = false" class="px-4 py-2.5 text-gray-600 hover:text-gray-800 text-sm">Annuler</button>
            </div>
        </form>
    </div>
</div>
