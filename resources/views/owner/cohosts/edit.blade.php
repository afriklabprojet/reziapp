...
@section('title', 'Modifier les permissions — ' . $cohost->name . ' - REZI')
        ...
        <form action="{{ route('owner.cohosts.update', [$residence, $cohost]) }}" method="POST" class="space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="space-y-3">
                    ...
                    @foreach ($permissions as $key => $perm)
                        ...
                        <label
                            class="flex items-start gap-4 p-4 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50 has-checked:border-orange-500 has-checked:bg-orange-50/50 transition">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                <textarea id="notes" name="notes" rows="3" placeholder="Notes internes..."
                    class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition resize-y">{{ old('notes', $cohost->notes) }}</textarea>
            </div>
