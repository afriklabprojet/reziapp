...
class SubscriptionController extends Controller
{
    ...
    public function subscribe(Request $request, SubscriptionPlan $plan)
    {
        return redirect()->route('owner.marketing.subscriptions.index')
            ->with('info', 'Rezi App ne fonctionne pas avec des abonnements. La plateforme prélève uniquement 10% sur le montant total de chaque réservation, côté propriétaire.');
    }
    ...
    {
        return redirect()->route('owner.marketing.subscriptions.index')
            ->with('info', 'Aucun changement de plan n\'est nécessaire: Rezi App applique un modèle unique de commission de 10% sur chaque réservation propriétaire.');
    }
    ...
    {
        return redirect()->route('owner.marketing.subscriptions.index')
            ->with('info', 'Aucun abonnement à annuler: Rezi App facture uniquement une commission de 10% sur les réservations encaissées par le propriétaire.');
    }
    ...
    {
        return redirect()->route('owner.marketing.subscriptions.index')
            ->with('info', 'Rezi App n\'utilise pas de réactivation d\'abonnement. Le modèle économique repose sur une commission propriétaire de 10% par réservation.');
    }
    ...
    {
        return redirect()->route('owner.marketing.subscriptions.index')
            ->with('info', 'Aucun paiement d\'abonnement n\'est attendu. Rezi App prélève directement 10% sur le montant total de chaque réservation propriétaire.');
    }
    ...
    {
        return redirect()->route('owner.marketing.subscriptions.index')
            ->with('info', 'Cette page d\'erreur d\'abonnement n\'est plus utilisée. Le modèle Rezi App est une commission propriétaire de 10% par réservation.');
    }
}
